...
object HandleGenerator {
    ...
    fun generateHandles(companionBuilder: TypeSpec.Builder, meta: XrossClass) {
        ...
        meta.fields.forEach {
            companionBuilder.addProperty(PropertySpec.builder("OFFSET_${it.name}", ClassName("", "FieldMemoryInfo"), KModifier.PRIVATE).build())
        }
        ...
        val init = CodeBlock.builder()
            ...
            .apply {
                // xross_free_string
                addStatement("xross_free_stringHandle = linker.downcallHandle(lookup.find(\"xross_free_string\").get(), FunctionDescriptor.ofVoid(%M))", ADDRESS)
                ...
                coreHandles.forEach { suffix ->
                    ...
                    val desc = when (suffix) {
                        ...
                        "clone" -> CodeBlock.of("FunctionDescriptor.of(%M, %M)", ADDRESS, ADDRESS)
                        "new" -> {
            ...
            .apply {
                meta.fields.forEach { addStatement("OFFSET_${it.name} = tempMap[%S] ?: throw IllegalStateException(%S)", it.name, "Field ${it.name} not found") }
            }
