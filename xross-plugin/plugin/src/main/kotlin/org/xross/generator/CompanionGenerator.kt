...
object CompanionGenerator {
    ...
    private val JAVA_BYTE = MemberName(VAL_LAYOUT, "JAVA_BYTE")
    ...
    fun generateCompanions(companionBuilder: TypeSpec.Builder, meta: XrossDefinition) {
        ...
        val init = CodeBlock.builder().addStatement("val linker = %T.nativeLinker()", Linker::class.asTypeName())
        ...
        init.beginControlFlow("try").addStatement("layoutRaw = layoutHandle.invokeExact() as %T", MEMORY_SEGMENT)
            ...
            .endControlFlow().nextControlFlow("catch (e: %T)", Throwable::class.asTypeName())
            .addStatement("throw %T(e)", RuntimeException::class.asTypeName()).endControlFlow()

        init.beginControlFlow("if (layoutStr.isNotEmpty())").addStatement("val parts = layoutStr.split(';')")
        ...
        init.beginControlFlow("if (layoutRaw != %T.NULL)", MEMORY_SEGMENT)
            .addStatement("xrossFreeStringHandle.invokeExact(layoutRaw)").endControlFlow().nextControlFlow("else")
